//! T1 BLE 专用 IMA/DVI ADPCM 解码（自包含副本，不依赖 `bridges::xiaomi`）
//!
//! 输入：ATVV 16 kHz 4-bit ADPCM；输出：PCM i16。
...
/// 16 kHz PCM → VB-CABLE 用的 48 kHz int16 LE（每输入采样 3 个输出）。
pub fn encode_16k_to_cable_le(samples: &[i16], prev: Option<i16>) -> (Vec<u8>, Option<i16>) {
    let mut out = Vec::with_capacity(samples.len() * 3 * 2);
    let mut last = prev;
    for &s in samples {
        let a = if let Some(p) = last {
            ((p as i32 + s as i32) / 2) as i16
        } else {
            s
        };
        last = Some(s);
